  ...
  const runPipeline = async (channel: ChannelConfig) => {
    ...
    try {
      ...
      try {
        ...
        throw new Error(`API 響應格式錯誤 (非 JSON): ${text.substring(0, 100)}...`);
      }

      if (!d1.success) throw new Error(d1.error);
      
      addLog(`🧠 [${channel.name}] 策略生成完成：${d1.metadata.strategy_note}`);
      updateChannel(channel.id, { step: 30, lastLog: '影片生成任務已提交至 Veo...' });
      ...
      try {
        ...
        throw new Error(`渲染 API 響應格式錯誤: ${text2.substring(0, 100)}...`);
      }
